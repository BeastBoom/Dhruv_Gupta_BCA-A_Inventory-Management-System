...
async function fetchCustomers() {
  try {
    const response = await fetch('http://inventory-management-system-production-83f5.up.railway.app/api/customers');
...
async function deleteCustomer(customerId) {
  try {
    const response = await fetch(`http://inventory-management-system-production-83f5.up.railway.app/api/customers/${customerId}`, {
      method: 'DELETE'
    });
    if (response.ok) {
...
document.getElementById("customerForm").addEventListener("submit", function(e) {
  ...
  if (editingCustomer) {
    fetch(`http://inventory-management-system-production-83f5.up.railway.app/api/customers/${editingCustomer.getAttribute('data-id')}`, {
      method: 'PUT',
    ...
    fetch("http://inventory-management-system-production-83f5.up.railway.app/api/customers", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(customerData)
    })
